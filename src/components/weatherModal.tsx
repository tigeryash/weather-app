...
const WeatherModal = ({chosen, isOpen, closeModal}: ModalProps) => {
    const { locations, setLocations,setIsInputFocused } = useLocationContext()
    const {data, isFetching, isSuccess} = useWeatherForSavedLocation(chosen as locationSaved)
                    ...
                    {isOpen && (
                        ...
                        >
                            <div className='flex justify-between'>
                                ...
                                {!locations.some(location => location.city === chosen?.city && location.country === chosen?.country) && <button
                                    className='text-white text-lg'
                                    onClick={addData}
                                >
                                    Add
                            ...
                            {isSuccess && 'weather' in weatherData && (
                                <>
                                    ...
                                    <div
                                        className='grid grid-cols-2 gap-6 w-full px-20'
                                    >
